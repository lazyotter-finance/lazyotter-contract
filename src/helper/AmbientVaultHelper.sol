...
contract AmbientVaultHelper is ReentrancyGuard {
    ...
    function previewAmountByAsset(
        address vault,
        uint256 assets
    ) public view returns (uint256 quoteTokenAmount, uint256 baseTokenAmount) {
    ...
    function previewSingleAmountByAsset(
        address vault,
        uint256 assets,
        bool isBase
    ...
    function previewSingleAmountByShare(
        address vault,
        uint256 shares,
        bool isBase
    ) public view returns (uint256 tokenAmount) {
    ...
    function deposit(
        ...
        _zap(normalizedInputs, limitPrice, poolInfo, minOut, IVault(vault).scaleFactor());
    ...
    function redeem(
        ...
        bytes memory returnMsg = _removeLiquidity(
            poolInfo,
            _safeConvertUint256ToUint128(_getRealLiquidity(vault, liquidity)),
            limitPrice,
            vault
        );
    ...
    function _zap(
        ...
        if (swap0To1) {
            deltaX =
                Zap.getDeltaX(
                    poolInfo.quoteTokenAmount / scaleFactor,
                    poolInfo.baseTokenAmount / scaleFactor,
                    inputs[0].amount / scaleFactor,
                    inputs[1].amount / scaleFactor,
                    swapFee,
                    MAX_FEE
                ) *
                scaleFactor;
    ...
    function _getRealLiquidity(address vault, uint256 liquidity) private view returns (uint256) {
        ICrocQuery.CurveState memory curve = crocQuery.queryCurve(
            IVault(vault).baseToken(),
            IVault(vault).quoteToken(),
            420
        );
